...
**(e) 역 라벨 부분 채색.** 여러 노선이 각자 다른 DB 를 쓰는데 역을 넷으로 쪼개면 지저분해진다.
...
`%%metro off_track: <station>` 으로 그 역을 본선 위로 띄우면, 들르는 실행은 위로 부풀고
안 들르는 실행은 본선으로 곧장 지나간다. `merge 안 함` 같은 역을 따로 만들면
하지 않는 일에 이름을 붙이게 되고, 역이 늘어 지도가 무거워진다.
