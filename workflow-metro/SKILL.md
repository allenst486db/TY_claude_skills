...
한 노선은 한 경로만 지난다. 그래서 같은 노선에 "역을 들르는 엣지"와 "건너뛰는 엣지"를
둘 다 걸어도 우회로로 그려지지 않는다 — nf-metro 가 한쪽만 고른다.
**우회는 별도 노선으로 그린다.** 짧은 구간만 도는 노선도 괜찮다:

```
%%metro line: nomerge | merge = no — skips MERGE | #7a7a7a | dashed
    fastqc -->|nomerge| trinity      # merge 역을 지나지 않는 한 개짜리 엣지
```

트랙 순서는 `%%metro line:` 선언 순서다. **바깥 트랙이 부풀고 안쪽이 직선**이 되므로,
본선에 두고 싶은 노선을 먼저 선언한다. 우회 노선을 나중에 선언하면 본선(종 노선)이
역에 직선으로 정차하고 우회로가 아래로 부푼다. 위로 부풀리려면 우회 노선을 먼저
선언해야 하는데, 그러면 이번엔 그 노선이 직선이 되고 역이 아래로 내려간다 —
둘을 동시에 만족시킬 수는 없으니 어느 쪽이 중요한지 정하고 간다.
