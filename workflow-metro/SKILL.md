...
**역은 직선 위에 두고 우회로를 위로 그리려면** 짧은 별도 노선을 만들고
`postprocess_svg.py --mirror-line <id>[:배율]` 로 뒤집는다. nf-metro 는 바깥 트랙을 항상
아래로 부풀리기 때문이다. 배율을 1.0 으로 두면 영역 상자 위로 삐져나간다 —
위쪽은 역 라벨이 차지해 아래보다 여유가 좁으므로 **0.3 안팎**으로 낮춰 라벨과 역 사이에 끼운다.
